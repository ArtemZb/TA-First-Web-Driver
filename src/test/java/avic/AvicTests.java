...
public class AvicTests {
    ...
    *****/
    ...
    public void checkItemsAreSortedInAscendingOrder(){
        driver.findElement(xpath("//span[@class='sidebar-item']")).click(); //open Catalog
        driver.findElement(xpath("//ul[contains(@class,'sidebar-list')]//a[contains(@href, 'elektronika')]")).click(); // click on Computers
        driver.findElement(xpath("//div[@class='brand-box__title']/a[contains(@href,'myishi')]")).click(); // click on Mouse category
        ...
        Select selectValue = new Select(driver.findElement(xpath("//div [label[text() ='Сортировать']]//select[@name= 'sort']")));
        selectValue.selectByValue("priceasc");
        new WebDriverWait(driver, 30).until(
                webDriver -> ((JavascriptExecutor) webDriver).executeScript("return document.readyState").equals("complete"));//wait for page loading
        List<WebElement> itemPrices = driver.findElements(xpath("//div[@class='prod-cart__prise-new']"));
        for (int i= 0; i< itemPrices.size()-1; i++){
            String currentItemPrice =  itemPrices.get(i).getText();
            String nextItemPrice = itemPrices.get(i+1).getText();
    ...
    public void checkForgotPasswordFormContainsEmailField(){
        driver.findElement(xpath("//a[contains(@href, 'sign-in')]//div[contains(@class, 'login')]")).click(); // open Sing-In page
        driver.findElement(xpath("//a[contains(@href, 'forgot')]")).click(); // select Forgot Password option
        WebDriverWait waitForgotPassword = new WebDriverWait(driver,20);
        waitForgotPassword.until(ExpectedConditions.visibilityOfElementLocated(id("js_forgotPassword")));
        assertTrue(driver.findElement(xpath("//form[contains(@action, 'forgot')]//input[@name='login']")).isDisplayed());
    }
    @Test(priority = 2)
    public void checkInstagramPageOpenedInNewTab(){
        driver.findElement(xpath("//li//a[contains(@href,'instagram')]")).click();
        WebDriverWait waitInstagramTab = new WebDriverWait (driver, 20);
        waitInstagramTab.until(ExpectedConditions.numberOfWindowsToBe(2));
        List<String> browserTabs = new ArrayList<String>(driver.getWindowHandles());
        driver.switchTo().window(browserTabs.get(1));
        assertTrue(driver.getCurrentUrl().equals("https://www.instagram.com/avicshop/"));
        driver.close();
    ...
    public void checkContactsBoxIsDisplayed(){
        WebElement contactsBox = driver.findElement(xpath("//div[@class ='header-top__item js-cont-btn']"));
        Actions builder = new Actions(driver);
        builder.moveToElement(contactsBox).build().perform();
        assertTrue(driver.findElement(xpath("//div[contains (@class,'header-top__item js-cont-btn')]")).getAttribute("class").contains("visible"));
    }
